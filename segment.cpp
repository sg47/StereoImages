...
CImg<double> naive_segment(const CImg<double> &img, const vector<Point> &fg, const vector<Point> &bg)
{
  ...
  double cost = 0;
    ...
    for(k=0; k<3; k++)
    {
		  tot[k] = tot[k] + img(fg[i].col,fg[i].row,0,k);
    }
	}

  for(k=0; k<3; k++)
  {
    mean[k] = tot[k]/fg_length;
  }
    ...
    for(k=0; k<3; k++)
    {
		  tot[k] = tot[k] + pow((img(fg[i].col,fg[i].row,0,k) - mean[k]),2);
		}
	}
	
  for(k=0; k<3; k++)
  {
	 var[k] = tot[k]/fg_length;
	}
    ...
    for(k=0; k<3; k++)
    {
		  result(fg[i].col,fg[i].row,0,k) = 1;
		}
	}
    ...
    for(k=0; k<3; k++)
    {
		  result(bg[i].col,bg[i].row,0,k) = 0;
		}
	}
